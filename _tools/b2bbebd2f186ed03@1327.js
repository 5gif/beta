// https://observablehq.com/@mootari/range-slider@1327
...
function _theme(jInputs,themes){return(
jInputs.radio({
...
function _color(jInputs){return(
jInputs.color({
...
function _rangeSlider(rangeInput,jInputs,html){return(
function rangeSlider(options = {}) {
  ...
  const output  = typeof format === 'function' ? format : jInputs.d3format.format(format);
...
function _rangeInput(theme_Flat,randomScope,DOM,html,Event,clamp,invalidation){return(
...
function _jInputs(lazyImport){return(
lazyImport('@jashkenas/inputs@2173', ['input', 'd3format', 'color', 'radio'])
)}
...
function _40(testInterval){return(
testInterval
)}
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("viewof theme")).define("viewof theme", ["jInputs","themes"], _theme);
  main.variable(observer("theme")).define("theme", ["Generators", "viewof theme"], (G, _) => G.input(_));
  main.variable(observer("viewof color")).define("viewof color", ["jInputs"], _color);
  ...
  main.variable(observer("rangeSlider")).define("rangeSlider", ["rangeInput","jInputs","html"], _rangeSlider);
  main.variable(observer("doc_rangeInput")).define("doc_rangeInput", ["signature","rangeInput","md"], _doc_rangeInput);
  main.variable(observer("rangeInput")).define("rangeInput", ["theme_Flat","randomScope","DOM","html","Event","clamp","invalidation"], _rangeInput);
  ...
  main.variable(observer("jInputs")).define("jInputs", ["lazyImport"], _jInputs);
  ...
  main.variable(observer()).define(["foo"], _38);
  main.variable(observer()).define(["viewof foo"], _39);
  main.variable(observer()).define(["testInterval"], _40);
